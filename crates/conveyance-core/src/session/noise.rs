...
//!
//! Thinness is deliberate. This file adds no crypto, no buffering, and
...
//! possible snow failure into [`ConveyanceError::HandshakeFailed`].
//!
...
//! buffers) is zeroized through our own types -- see `Session::end`.
...
impl SessionHandshake {
    ...
    pub fn write_message(&mut self, payload: &[u8]) -> Result<Vec<u8>, ConveyanceError> {
        ...
        let n = self
            ...
            .map_err(|_| ConveyanceError::HandshakeFailed)?;
        out.truncate(n);
        Ok(out)
    }
    ...
    pub fn into_transport(self) -> Result<SessionTransport, ConveyanceError> {
        let transport = self
            .inner
            .into_transport_mode()
            .map_err(|_| ConveyanceError::HandshakeFailed)?;
...
impl SessionTransport {
    ...
    pub fn send(&mut self, plaintext: &[u8]) -> Result<Vec<u8>, ConveyanceError> {
        ...
        let n = self
            ...
            .map_err(|_| ConveyanceError::SessionEnded)?;
        out.truncate(n);
        Ok(out)
    }
    ...
    /// so this returns `ConveyanceError::SessionEnded` (generic, leaks
...
fn handshake_failed(_: snow::Error) -> ConveyanceError {
...
mod tests {
    ...
    use crate::crypto::{EntropySource, OsEntropy, Secret};
    ...
    fn wrong_remote_static_fails_cleanly_in_both_configurations() {
        ...
        match resp.read_message(&m1) {
            Err(ConveyanceError::HandshakeFailed) => {} // generic: exactly right
        ...
        match resp2.read_message(&m1b) {
            Err(ConveyanceError::HandshakeFailed) => {}
            other => panic!("expected generic HandshakeFailed, got {other:?}"),
        }

        // And neither failure leaks anything beyond the generic code.
        let probe = ConveyanceError::HandshakeFailed;
